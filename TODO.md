...
- [~] Premium-mallar/-arbetsflöden + MCP till premium + export av premium — **innehåll klart, UI/MCP kvar.**
  ...
  - [ ] **Kolla: kommer Pro-mallarna ut i den hostade `mcp_promptbanken`-servern?** `get_pro_templates_for_mcp_key`-RPC:n och `list_pro_templates`-verktyget ovan gäller bara den **lokala** stdio-MCP-servern i det här repot (kräver att användaren kör den själv med rätt env-variabler). Den publika/hostade servern på `https://mcp.promptbanken.se/sse` är en **separat drift** (`mcp_promptbanken` på VPS:en, egen kodbas utanför detta repo) och har inte fått motsvarande kod — måste verifieras/byggas där separat om Pro-mallarna ska kunna nås via den publika MCP-adressen också.
