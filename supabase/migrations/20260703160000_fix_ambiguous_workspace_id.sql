-- Bugfix: "column reference workspace_id is ambiguous".
--
-- invite_org_member() och redeem_org_join_code() har båda en OUT-parameter
-- (returns table(..., workspace_id uuid, ...)) som PL/pgSQL exponerar som
-- en implicit variabel i funktionskroppen. Där en WHERE-sats sedan skrev
-- den obekväliserade kolumnen "workspace_id" (t.ex. "where ... and
-- workspace_id = ...") blev det tvetydigt mellan variabeln och
-- public.profiles.workspace_id-kolumnen. Fix: tabellalias + kvalificerade
-- kolumnnamn överallt.
    ...
    if exists (
        select 1
         ...
         where p.user_id = current_user_id
           and p.workspace_id = code_record.workspace_id
    ) then
